...
class SerialSnooper:
    ...
    def __init__(self, port, baud=19200, timeout=0):
        ...
        self.connection = serial.Serial(port=port, baudrate=baud, bytesize=serial.EIGHTBITS,
                                        parity=serial.PARITY_EVEN, stopbits=serial.STOPBITS_ONE, timeout=timeout)
        log.info("Opening serial interface: \n" + "\tport: {} \n".format(port) + "\tbaudrate: {}\n".format(baud) + "\tbytesize: {}\n".format(self.connection.bytesize) + "\tparity:{}\n".format(self.connection.parity) +
                 "\tstopbits:{}\n".format(self.connection.stopbits) + "\ttimeout: {}\n".format(timeout))
            ...
            if len(modbusdata) > (frameStartIndex + 2):
                ...
                if functionCode in (1, 2):
                    ...
                    if len(modbusdata) >= (frameStartIndex + expectedLenght):
                        ...
                        # Read Address (2)
                        readAddress = (
                            modbusdata[bufferIndex] * 0x0100) + modbusdata[bufferIndex + 1]
                        bufferIndex += 2
                        # Read Quantity (2)
                        readQuantity = (
                            modbusdata[bufferIndex] * 0x0100) + modbusdata[bufferIndex + 1]
                        bufferIndex += 2
                        # CRC16 (2)
                        crc16 = (modbusdata[bufferIndex] *
                                 0x0100) + modbusdata[bufferIndex + 1]
                        ...
                        if crc16 == metCRC16:
                            ...
                            log.info("Master\t\t-> ID: {}, {}: 0x{:02x}, Read address: {}, Read Quantity: {}".format(
                    ...
                    if (request == False):
                        ...
                        if len(modbusdata) >= (frameStartIndex + expectedLenght):
                            ...
                            if len(modbusdata) >= (frameStartIndex + expectedLenght):
                                ...
                                crc16 = (
                                    modbusdata[bufferIndex] * 0x0100) + modbusdata[bufferIndex + 1]
                                metCRC16 = self.calcCRC16(
                                    modbusdata, bufferIndex)
                                bufferIndex += 2
                                if crc16 == metCRC16:
                                    if self.trashdata:
                                        self.trashdata = False
                                    ...
                                    log.info("Slave\t-> ID: {}, {}: 0x{:02x}, Read byte count: {}, Read data: [{}]".format(
                                        unitIdentifier, functionCodeMessage, functionCode, readByteCount, " ".join(["{:02x}".format(x) for x in readData])))
                                    modbusdata = modbusdata[bufferIndex:]
                                    bufferIndex = 0
                            else:
                                needMoreData = True
                        ...
                        if len(modbusdata) >= (frameStartIndex + expectedLenght):
                            ...
                            if crc16 == metCRC16:
                                ...
                                if functionCode == 1:
                                    ...
                                    functionCodeMessage = 'Read Discrete Inputs'
                                log.info("Slave\t-> ID: {}, {}: {}, Exception: {}".format(
                                    unitIdentifier, functionCodeMessage, functionCode, exceptionCode))
                                modbusdata = modbusdata[bufferIndex:]
                                bufferIndex = 0
                        else:
                            needMoreData = True
                    ...
                    if len(modbusdata) >= (frameStartIndex + expectedLenght):
                        ...
                        if crc16 == metCRC16:
                            ...
                            log.info("Master\t-> ID: {}, Write Single Coil: 0x{:02x}, Write address: {}, Write data: [{}]".format(
                                unitIdentifier, functionCode, writeAddress, " ".join(["{:02x}".format(x) for x in writeData])))
                            modbusdata = modbusdata[bufferIndex:]
                            bufferIndex = 0
                    else:
                        needMoreData = True
                    ...
                    if (request == False):
                        ...
                        if len(modbusdata) >= (frameStartIndex + expectedLenght):
                            ...
                            if crc16 == metCRC16:
                                ...
                                error = False
                                log.info(
                                    "Slave\t-> {} {} {}".format(unitIdentifier, functionCode, writeAddress))
                                modbusdata = modbusdata[bufferIndex:]
                                bufferIndex = 0
                        else:
                            needMoreData = True
                    ...
                    if (request == False) & (responce == False):
                        ...
                        if len(modbusdata) >= (frameStartIndex + expectedLenght):
                            ...
                            if crc16 == metCRC16:
                                ...
                                responce = False
                                error = True
                                log.info("Slave\t-> ID: {}, Write Single Coil: {}, Exception: {}".format(
                                    unitIdentifier, functionCode, exceptionCode))
                                modbusdata = modbusdata[bufferIndex:]
                                bufferIndex = 0
                        else:
                            needMoreData = True
                    ...
                    if (request == False):
                        ...
                        if len(modbusdata) >= (frameStartIndex + expectedLenght):
                            ...
                            if crc16 == metCRC16:
                                ...
                                responce = True
                                error = False
                                log.info("Slave\t-> ID: {}, Write Single Register: 0x{:02x}, Write address: {}, Write data: [{}]".format(
                                    unitIdentifier, functionCode, writeAddress, " ".join(["{:02x}".format(x) for x in writeData])))
                                modbusdata = modbusdata[bufferIndex:]
                                bufferIndex = 0
                        else:
                    ...
                    if (request == False) & (responce == False):
                        ...
                        if len(modbusdata) >= (frameStartIndex + expectedLenght):
                            ...
                            if crc16 == metCRC16:
                                ...
                                responce = False
                                error = True
                                log.info("Slave\t-> ID: {}, Write Single Register: {}, Exception: {}".format(
                                    unitIdentifier, functionCode, exceptionCode))
                                modbusdata = modbusdata[bufferIndex:]
                                bufferIndex = 0
                        else:
                        ...
                        if len(modbusdata) >= (frameStartIndex + expectedLenght):
                            ...
                            if crc16 == metCRC16:
                                ...
                                log.info("Master\t-> ID: {}, Write Multiple Coils: 0x{:02x}, Write address: {}, Write quantity: {}, Write data: [{}]".format(
                                    unitIdentifier, functionCode, writeAddress, writeQuantity, " ".join(["{:02x}".format(x) for x in writeData])))
                                modbusdata = modbusdata[bufferIndex:]
                                bufferIndex = 0
                        else:
                            needMoreData = True
                    ...
                    if (request == False):
                        ...
                        if len(modbusdata) >= (frameStartIndex + expectedLenght):
                            ...
                            if crc16 == metCRC16:
                                ...
                                responce = True
                                error = False
                                log.info("Slave\t-> ID: {}, Write Multiple Coils: 0x{:02x}, Write address: {}, Write Quantity: {}".format(
                    ...
                    if (request == False) & (responce == False):
                        ...
                        if len(modbusdata) >= (frameStartIndex + expectedLenght):
                            ...
                            if crc16 == metCRC16:
                                ...
                                responce = False
                                error = True
                                log.info("Slave\t-> ID: {}, Write Multiple Coils: {}, Exception: {}".format(
                                    unitIdentifier, functionCode, exceptionCode))
                                modbusdata = modbusdata[bufferIndex:]
                                bufferIndex = 0
                        else:
                    ...
                    if (request == False):
                        ...
                        if len(modbusdata) >= (frameStartIndex + expectedLenght):
                            ...
                            if crc16 == metCRC16:
                                ...
                                responce = True
                                error = False
                                log.info("Slave\t-> ID: {}, Write Multiple registers: 0x{:02x}, Write address: {}, Write quantity: {}".format(
                                    unitIdentifier, functionCode, writeAddress, writeQuantity))
                                modbusdata = modbusdata[bufferIndex:]
                                bufferIndex = 0
                        else:
                    ...
                    if (request == False) & (responce == False):
                        ...
                        if len(modbusdata) >= (frameStartIndex + expectedLenght):
                            ...
                            if crc16 == metCRC16:
                                ...
                                responce = False
                                error = True
                                log.info("Slave\t-> ID: {}, Write Multiple registers: 0x{:02x}, Exception: {}".format(
                                    unitIdentifier, functionCode, exceptionCode))
                                modbusdata = modbusdata[bufferIndex:]
                                bufferIndex = 0
                        else:
                        ...
                        if len(modbusdata) >= (frameStartIndex + expectedLenght):
                            ...
                            if crc16 == metCRC16:
                                ...
                                log.info("Master\t-> ID: {}, Read/Write Multiple registers: 0x{:02x}, Read address: {}, Read Quantity: {}, Write address: {}, Write quantity: {}, Write data: [{}]".format(
                                    unitIdentifier, functionCode, readAddress, writeAddress, readQuantity, writeQuantity, " ".join(["{:02x}".format(x) for x in writeData])))
                                modbusdata = modbusdata[bufferIndex:]
                                bufferIndex = 0
                        else:
                            needMoreData = True
                            ...
                            if len(modbusdata) >= (frameStartIndex + expectedLenght):
                                ...
                                if crc16 == metCRC16:
                                    ...
                                    log.info("Slave\t-> ID: {}, Read/Write Multiple registers: 0x{:02x}, Read byte count: {}, Read data: [{}]".format(
                                        unitIdentifier, functionCode, readByteCount, " ".join(["{:02x}".format(x) for x in readData])))
                                    modbusdata = modbusdata[bufferIndex:]
                                    bufferIndex = 0
                            else:
                                needMoreData = True
                    ...
                    if (request == False) & (responce == False):
                        ...
                        if len(modbusdata) >= (frameStartIndex + expectedLenght):
                            ...
                            if crc16 == metCRC16:
                                ...
                                responce = False
                                error = True
                                log.info("Slave\t-> ID: {}, Read/Write Multiple registers: {}, Exception: {}".format(
                                    unitIdentifier, functionCode, exceptionCode))
                                modbusdata = modbusdata[bufferIndex:]
                                bufferIndex = 0
                        else:
                ...
                # FC43 ( 0x2B) Encapsulated Interface Transport
                    ...
                    # Request size: UnitIdentifier (1) + FunctionCode (1) + MEI Type (1) + Read Device ID Code (1) + Object ID (1) + CRC (2)
                    expectedLenght = 7  # 7
                    if len(modbusdata) >= (frameStartIndex + expectedLenght):
                        bufferIndex = frameStartIndex + 2
                        # MEI Type (1)
                        readMEI = modbusdata[bufferIndex]
                        bufferIndex += 1
                        # Read DeviceIdCode (1)
                        readDeviceIdCode = modbusdata[bufferIndex]
                        bufferIndex += 1
                        # Object ID (1)
                        objectId = modbusdata[bufferIndex]
                        bufferIndex += 1
                        # CRC16 (2)
                        crc16 = (modbusdata[bufferIndex] *
                                 0x0100) + modbusdata[bufferIndex + 1]
                        ...
                        if crc16 == metCRC16:
                            ...
                            responce = False
                            error = False
                            functionCodeMessage = 'Encapsulated Interface Transport'
                            log.info("Master\t-> ID: {}, {} ({}), MEI: {}, Device ID Code: {}, Object ID: {}".format(
                                unitIdentifier, functionCodeMessage, functionCode, readMEI, readDeviceIdCode, objectId))
                            modbusdata = modbusdata[bufferIndex:]
                            bufferIndex = 0
                    else:
                        needMoreData = True
                    ...
                    if (request == False):
                        # Responce size: UnitIdentifier (1) + tbd + CRC (2)
                        # rough guess, Tool4Cool requests get answered by 34 bytes, will be advanced later in the loop
                        expectedLenght = 34
                        EITObjects = {}  # reserved for data of Objects
                        if len(modbusdata) >= (frameStartIndex + expectedLenght):
                            bufferIndex = frameStartIndex + 2
                            # MEI Type (1)
                            readMeiType = modbusdata[bufferIndex]
                            bufferIndex += 1
                            # Read Device ID Code (1)
                            readDeviceIdCode = modbusdata[bufferIndex]
                            bufferIndex += 1
                            # Conformity Level (1)
                            readConformityLevel = modbusdata[bufferIndex]
                            bufferIndex += 1
                            # More Follows (1)
                            # Functionality not implemented for now
                            readMoreFollows = modbusdata[bufferIndex]
                            bufferIndex += 1
                            # Next Object Id (1)
                            readNextObjectId = modbusdata[bufferIndex]
                            bufferIndex += 1
                            # Number of Objects (1)
                            readNumberOfObjects = modbusdata[bufferIndex]
                            bufferIndex += 1
                            # -------- Objects
                            # decode the objects
                            # buffer the amount of read objects to report later how many came
                            objBuffer = readNumberOfObjects
                            while (objBuffer > 0):
                                eitobject = []
                                # Obj Id (1)
                                objId = modbusdata[bufferIndex]
                                bufferIndex += 1
                                # Obj Size (in byte, 1)
                                objSize = modbusdata[bufferIndex]
                                bufferIndex += 1
                                while (objSize > 0):
                                    # check if the expected byte length is still bigger than the current frame
                                    # +2 for CRC
                                    if len(modbusdata) < (frameStartIndex + expectedLenght + 2):
                                        expectedLenght += 1  # extend the loop for another round
                                    eitobject.append(modbusdata[bufferIndex])
                                    bufferIndex += 1
                                    objSize -= 1
                                # store the objects payload to the EIT objects list
                                EITObjects[objId] = eitobject
                                objBuffer -= 1
                            ...
                            if crc16 == metCRC16:
                                ...
                                # decode the EITObjects Dictonary
                                # known object ids:         type
                                # 00: Vendor Name          ASCII
                                # 01: Product Code         ASCII
                                # 02: MajorMinorRevision   ASCII
                                # 128: reserved
                                # 129: reserved

                                if 0 in EITObjects.keys():
                                    EITObjects['VendorName (0x00)'] = bytearray.fromhex(
                                        "".join(["{:02x}".format(x) for x in EITObjects[0]])).decode()
                                    del (EITObjects[0])
                                if 1 in EITObjects.keys():
                                    EITObjects['ProductCode (0x01)'] = bytearray.fromhex(
                                        "".join(["{:02x}".format(x) for x in EITObjects[1]])).decode()
                                    del (EITObjects[1])
                                if 2 in EITObjects.keys():
                                    EITObjects['MajorMinorRevision (0x02)'] = bytearray.fromhex(
                                        "".join(["{:02x}".format(x) for x in EITObjects[2]])).decode()
                                    del (EITObjects[2])
                                if 128 in EITObjects.keys():
                                    EITObjects['reserved (0xF3)'] = "reserved"
                                    del (EITObjects[128])
                                if 129 in EITObjects.keys():
                                    EITObjects['reserved (0xF4)'] = "reserved"
                                    del (EITObjects[129])

                                functionCodeMessage = 'Encapsulated Interface Transport'
                                log.info("Slave\t-> ID: {}, {}: {}, Read byte count: {}, Number of Objects: {} Read data: [{}]".format(
                                    unitIdentifier, functionCodeMessage, functionCode, readByteCount, readNumberOfObjects, EITObjects))
                                modbusdata = modbusdata[bufferIndex:]
                                bufferIndex = 0
                            else:
                                needMoreData = True
                    ...
                    if (request == False) & (responce == False):
                        ...
                        if len(modbusdata) >= (frameStartIndex + expectedLenght):
                            ...
                            if crc16 == metCRC16:
                                ...
                                responce = False
                                error = True
                                log.info("Slave\t-> ID: {}, Exception: 0x{:02x}, Code: {}".format(
                                    unitIdentifier, functionCode, exceptionCode))
                                modbusdata = modbusdata[bufferIndex:]
                                bufferIndex = 0
                        else:
